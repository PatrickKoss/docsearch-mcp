  ...
  describe('ingestConfluence', () => {
    ...
    it('should fetch and ingest pages from configured spaces', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve(mockPageResponse('page123', 'Test Page 1', '<h1>Hello World</h1><p>Content here</p>'))
        })
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve(mockPageResponse('page456', 'Test Page 2', '<h2>Another Page</h2><p>More content</p>'))
        })
        .mockResolvedValueOnce({
          ok: true,
    ...
    it('should convert HTML to markdown', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve(mockPageResponse('page123', 'HTML Page', '<h1>Title</h1><p><strong>Bold</strong> and <em>italic</em> text</p>'))
        })
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve(mockPageResponse('page456', 'Simple Page', '<p>Simple content</p>'))
        })
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ results: [], _links: {} })
    ...
    it('should handle pagination', async () => {
      ...
      const documents = db.prepare('SELECT COUNT(*) as count FROM documents WHERE source = ?').get('confluence')
    ...
    it('should handle pages with alternative ID structure', async () => {
      ...
      mockFetch
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve(mockPageResponse('direct123', 'Direct ID Page', '<p>Direct content</p>'))
        })
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ results: [], _links: {} })
