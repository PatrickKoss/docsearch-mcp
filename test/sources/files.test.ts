...
module.exports = config;`
  ...
  describe('ingestFiles', () => {
    ...
    it('should create chunks for ingested files', async () => {
      ...
      const chunkWithContent = db.prepare("SELECT * FROM chunks WHERE content LIKE '%hello%'").get()
    ...
    it('should detect file changes and re-chunk', async () => {
      ...
      const originalChunks = db.prepare('SELECT * FROM chunks JOIN documents ON chunks.document_id = documents.id WHERE documents.uri LIKE ?').all('%sample.ts%')
      ...
      const newChunks = db.prepare('SELECT * FROM chunks JOIN documents ON chunks.document_id = documents.id WHERE documents.uri LIKE ?').all('%sample.ts%')
  ...
  describe('File type detection', () => {
    it('should identify code files correctly', async () => {
      ...
      const codeFiles = db.prepare("SELECT * FROM documents WHERE lang IN ('ts', 'js', 'py')").all()
