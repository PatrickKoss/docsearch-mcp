...
describe('Configuration', () => {
  ...
  describe('File configuration', () => {
    it('should use default file patterns', async () => {
      ...
      expect(CONFIG.FILE_ROOTS).toEqual(['.'])
      expect(CONFIG.FILE_INCLUDE_GLOBS).toEqual(['**/*.{go', 'ts', 'tsx', 'js', 'py', 'rs', 'java', 'md', 'mdx', 'txt', 'yaml', 'yml', 'json}'])  // Default split on commas
      expect(CONFIG.FILE_EXCLUDE_GLOBS).toEqual(['**/{.git', 'node_modules', 'dist', 'build', 'target}/**'])  // Default split on commas
    })
