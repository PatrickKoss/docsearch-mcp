...
describe('Configuration', () => {
  ...
  describe('Environment variable parsing', () => {
    it('should use default values when environment variables are not set', async () => {
      ...
      const { CONFIG } = await import('../src/shared/config/legacy-config.js');
    ...
    it('should validate embeddings provider', async () => {
      ...
      vi.resetModules();
      process.env.EMBEDDINGS_PROVIDER = 'tei';
      const { CONFIG: CONFIG2 } = await import('../src/shared/config/legacy-config.js');
      expect(CONFIG2.EMBEDDINGS_PROVIDER).toBe('tei');

      vi.resetModules();
      process.env.EMBEDDINGS_PROVIDER = 'openai';
      const { CONFIG: CONFIG3 } = await import('../src/shared/config/legacy-config.js');
  ...
  describe('CSV parsing utility', () => {
    it('should handle single values', async () => {
      ...
      const { CONFIG: _CONFIG } = await import('../src/shared/config/legacy-config.js');
